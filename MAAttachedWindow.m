...
//
// license at: http://mattgemmell.com/license
...
- (NSPoint)point
{
    return NSMakePoint(NSMidX([_anchor frame]), NSMidY([_anchor frame]));
}
...
- (MAAttachedWindow*)initWithContentView:(NSView*)view attachedToView:(NSView*)anchor onSide:(MAWindowPosition)side
{
    float distance = 0;
    switch (side) {
        case MAPositionLeft:
        ...
        case MAPositionRightBottom:
            distance = [anchor frame].size.width/2+3;
            break;
        default:
            distance = [anchor frame].size.height/2+3;
            break;
    }
    self = [self initWithView:view
               attachedToView:anchor 
                     inWindow:[anchor window] 
                       onSide:side 
                   atDistance:distance];
    return self;
}
...
- (MAAttachedWindow *)initWithView:(NSView *)view 
                    attachedToView:(NSView*)anchor
                        ...
                        atDistance:(float)distance
{
    return [self initWithView:view attachedToView:anchor 
...
- (void)_updateGeometry
{
    ...
    contentRect.origin = (_window) ? [_window convertBaseToScreen:[self point]] : [self point];
...
- (MAWindowPosition)_bestSideForAutomaticPosition
{
    ...
    NSPoint pointOnScreen = (_window) ? [_window convertBaseToScreen:[self point]] : [self point];
